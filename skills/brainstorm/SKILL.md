...
## v2 — Guardrail (chống bịa)
- Số liệu/khẳng định về hệ thống/thị trường phải có nguồn hoặc gắn `[chưa verify]`/`[giả định]` (khớp `.claude/shared/pushback-triggers.md` MEM-PBK-02).
- Không "phát minh" fact về VisibilityPRO/NDS — nếu cần dữ kiện nội bộ → `search_wiki` (NDS-Knowledge) hoặc flag để discovery-agent xác minh.

## v2 — Options Comparison block (light, KHÔNG gate)

Khi có **≥2 hướng khả thi**, xuất block chuẩn vào `03-brainstorm--notes.md` (đầu vào cho PRD "Quyết định đã chốt" + impact-trace):

```
## Options
| ID | Hướng | Mô tả 1 dòng | Ưu | Nhược | Effort | Risk |
|---|---|---|---|---|---|---|
| OPT-A | ... | ... | ... | ... | S/M/L | Thấp/TB/Cao |
| OPT-B | ... | ... | ... | ... | | |

**Recommended:** OPT-x — [lý do ngắn]
**Bác bỏ:** OPT-y — [lý do]
```
→ ba-lead append quyết định + Bác bỏ vào `00-decision-log.md`.
> **Chủ đích KHÔNG có options-gate** (không chặn PRD chờ chọn option như ba-kit-solo) — giữ autopilot của ba-lead. Options chỉ là artifact so sánh + provenance, không phải cổng fail-closed.
